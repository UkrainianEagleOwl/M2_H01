...
from src.input_module import get_command_input
from src.constants import *
from src.memory import Record,AddressBook,Phone
...
def input_error(func):
    def wrapper(*arg,a_book = AddressBook,n_book = Notebook):
        try:
            ...
            output_for_user.show_message(Fore.RED + "The contact is missing. Please try again" + Style.RESET_ALL)
        except IndexError:
            output_for_user.show_message(Fore.RED + "The contact is missing. Please try again" + Style.RESET_ALL) 
        except ValueError:
            output_for_user.show_message(Fore.RED + "ValueError. Please try again." + Style.RESET_ALL)
    return wrapper    
...
def find_user(*arg,a_book = AddressBook,n_book = Notebook):
    ...
    (output_for_user.show_contact(result) for result in results) if results else output_for_user.show_message("No matches found among contacts.")
